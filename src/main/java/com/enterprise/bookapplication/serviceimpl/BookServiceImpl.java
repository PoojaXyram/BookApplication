...
public class BookServiceImpl implements BookService {
  ...
  public BookResponse getAll(
      Integer pageNumber, Integer pageSize, String sortBy, String sortDirec) {
    Sort sort = null;
    ...
    List<BookDto> list =
        booksList
            .stream()
            .map(books -> this.modelMapper.map(books, BookDto.class))
  ...
  public List<BookDto> findByCategory(Integer id) {
    //        Category category=categoryDao.findById(id).orElseThrow(() -> new
    // ResourceNotFound("Author", id));
    List<BookDto> bookDtos =
        bookDao
            .findByCategoryId(id)
            .stream()
